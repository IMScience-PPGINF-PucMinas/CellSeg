...
class CellposeModel():
    ...
    def extract_activation_maps(
        ...
        """
        Run the same preprocessing as :meth:`eval` and return merged intermediate activations.

        Hooks either the encoder neck (``layer='neck'``) or the last 1x1 conv before
        upsampling (``layer='out'`` or ``'last_conv'``).

        Parameters
        ----------
        return_rgb : bool
            If True, also return a uint8 RGB preview (first three channels, min–max per channel).

        Returns
        -------
        act : ndarray
            Float array ``[nimg, Ly, Lx, C]`` (single plane: ``[Ly, Lx, C]`` if ``nimg==1``).
        act_rgb : ndarray or None
            uint8 ``[..., 3]`` preview, or None if ``return_rgb`` is False.
        """
        x = transforms.convert_image(x, channel_axis=channel_axis, z_axis=z_axis, do_3D=False)
        if x.ndim < 4:
            x = x[np.newaxis, ...]
        nimg = x.shape[0]
        ...
        if diameter is not None:
            ...
            x = transforms.resize_image(
                x, Ly=int(x.shape[1] * sc), Lx=int(x.shape[2] * sc)
            )
        ...
        do_normalization = True if normalize_params["normalize"] else False
        if nimg > 1 and do_normalization:
            normalize_params["norm3D"] = False
        if normalize_params["norm3D"] and nimg > 1 and do_normalization:
            models_logger.warning(
                "normalize_params['norm3D'] is True but stacking is 2D-only here; setting to False"
            )
            normalize_params["norm3D"] = False
        if do_normalization:
            x = transforms.normalize_img(x, **normalize_params)

        act = activation_maps.run_net_activation(
            self.net,
            x,
            layer=layer,
            batch_size=batch_size,
        ...
        if resample and diameter is not None:
            act = transforms.resize_image(act, Ly=Ly_0, Lx=Lx_0, no_channels=False)
        ...
        act_rgb = None
        if return_rgb:
            av = act[np.newaxis, ...] if act.ndim == 3 else act
            act_rgb = activation_maps.activation_volume_to_rgb(av, mode=rgb_mode)
            if act_rgb.shape[0] == 1:
                act_rgb = act_rgb[0]
